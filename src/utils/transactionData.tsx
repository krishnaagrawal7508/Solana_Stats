...
export const generateTransactionData = async (walletAddress: string) => {
  ...
  const connection = new Connection(
    'https://wispy-skilled-diamond.solana-mainnet.quiknode.pro/675c3e2fdf7c3d5426619c95a25c5521d408a6b7'
  );
  // const connection = new Connection(clusterApiUrl("mainnet-beta"));
  const publicKey = new PublicKey(walletAddress);

  let fetchedTransactions = await connection.getSignaturesForAddress(
    publicKey,
    { limit: 1000 }
  );
  ...
  while (fetchedTransactions.length === 1000) {
    ...
    fetchedTransactions = await connection.getSignaturesForAddress(publicKey, {
  ...
  // fDay with maximum transactions and return the count
  let max = 0;
  let maxDate = '';
  for (const [date, count] of Object.entries(filteredTransactionData)) {
    if (count > max) {
      max = count;
      maxDate = date;
    }
  ...
  return [formattedData, total_transactions, max_transactions.number];
};
